...
document.write(day + '</font><font size=2>' + myweekday + '</font>' + month + year + ' - ' + '<font size=2>' + timeValue + '</font>')
...
$login=@mysql_query("SELECT * FROM _tudo WHERE tituloPerfil LIKE'%$_POST[logar]%';",$conexao);
$login=@mysql_fetch_array($login);
$login=$login["email"];

$redireciona=@mysql_query("SELECT * FROM _tudo WHERE _tudo.email ='$login';",$conexao);
...
$listar4=@mysql_query("SELECT telefone FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
...
$listar16=@mysql_query("SELECT tipo FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar17=@mysql_query("SELECT hobby FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
...
//echo $cor;
#B5C0C3
echo'<table style="background:'.$cor.'; width:93%;  margin-top:0%; z-index:-90; border-radius:12px;"><tr><td>';
...
echo'<div  height="80" style="margin-left:0px;margin-top:15px; valign="bottom"><div id="campo2" style=" background-image:url(../BACKGROUNDS/CampoMyWeb.png);
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px;">Telefone:&nbsp;&nbsp;
<br/><input type="text" readonly="true" name="E-mail" size="30" valign="bottom" style="background-color:transparent; font-weight:600;" id="E-mail" value="'
...
id="hobbies" style="text-align:justify; margin:4px 0px 0px 0px;" scrolling="yes">' .@mysql_result($listar17,0,hobby);
echo'</textarea></div>
';
...
<?php
echo'<aside     style="background:#B0C6D2;z-index:999;   border-radius:10px; margin-top:20%; margin-left:-5%; postion:relative; float:left; ">
<p><font color="#06438E" size="3" face="verdana">
<b>Quem sou eu...</b></font></p>
<p>
<textarea name="texto" readonly="true"  valign="bottom"  id="Texto" wrap="on" cols="33" rows="10"
...
echo'</form>';
echo'</table>';
echo'<table  bgcolor="'.$cor.'" id="body" align="center"><tr>
<td colspan="3"> <br/>
<div id="formulario1">
...
if($adicionar > 0){ 
echo'<div style="background:'.$cor.'; width:206%; height:50px; border-radius:12px; margin-left:-62%;">';
 echo"<p style='margin-left:8px;'>Voces sao amigos.</p>";
 echo"</div>";
}
else{ 
    echo"<div style='background:'.$cor.'; width:206%; height:50px; border-radius:12px; margin-left:-62%;'>";
...
<?php
echo'<hr size="30%" width="90%" color="#A2A6A9">';
...
$listar4=@mysql_query("SELECT telefone FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
...
$listar16=@mysql_query("SELECT tipo FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar17=@mysql_query("SELECT hobby FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);

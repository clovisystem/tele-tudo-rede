...
$email=@mysql_query("SELECT email FROM users WHERE tituloPerfil='".$login."';",$conexao);
...
if ($tipo=="usuario"){
  ...
  $listar5=@mysql_query("SELECT telefone FROM _users WHERE tituloPerfil LIKE '%$sessao%';",$conexao);
...
$arquivo0 = $sql0['imagem'];*/
  ...
  echo'<table width="90%" bgcolor="'.$cor.'" align="center"><tr>
  ...
  echo'<div style=" margin-left:52%; border-radius:12px; background:#CCCCCC; float:left; position:relative; padding:4px 4px 4px 4px; text-transform:capitalize;"><center>'.$localizado1[0].'</center><br/>';
...
echo'<table style="background:'.$cor.'; width:97%;  margin-top:0%; z-index:-90; border-radius:12px;"><tr><td>';
echo'<article  align="left"  style=" background:'.$cor.'; z-index:999; height:230%;  width:80%; bordercolor:transparent; position:relative; float:left;  margin-top:2%; margin-left:0%; ">';
echo'<form>';

echo'';
echo'';
echo '<p style="margin-left:5px; color:gray;">'.str_replace("_"," ",$login).'<span style="font-size:18px; font-family:calibri,arial; color:green; font-weight:bold;">
, esta &eacute; a P&aacute;gina do perfil de <input type="text" name="logon" 
value="'.$perfil=str_replace("_"," ",$perfil).'" readonly="true" size="16" STYLE="background-color:transparent; 
border:none; font-size:18px; font-family:calibri,arial; color:green; font-weight:bold;"/></span>';
echo'&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="Voltar" onclick="history.go(-2);" /></p>';
echo'';


/*echo'<div style="margin-left:0px;margin-top:5px; " ><div id="campo" style="z-dex:999; background-image:url(BACKGROUNDS/CampoMyWeb.png);
...
background-repeat:no-repeat;height:160px;padding-top:20px;padding-left:10px; width:110%;">Minhas atividades:
...
background-repeat:no-repeat;height:160px;padding-top:20px;padding-left:10px; width:110%"M>Do que gosto de comprar:&nbsp;&nbsp;
...
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px; width:110%">Telefone:&nbsp;&nbsp;
...
value="'.@mysql_result($listar5,0,telefone);
echo'"/></div></div>
';
...
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px; width:110%">Estado:&nbsp;&nbsp;
...
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px; width:110%">Cidade:&nbsp;&nbsp;
...
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px; width:110%">Anivers&aacute;rio:&nbsp;&nbsp;
...
<?php
echo'<aside     style="background:#B0C6D2;z-index:999; border-radius:10px; margin-top:20%; postion:relative; float:left; ">
<p><font color="#06438E" size="3" face="verdana">
<b>Quem sou eu...</b></font></p>
<p>
<textarea name="texto" readonly="true"  valign="bottom"  id="Texto" wrap="on" cols="32.8" rows="10"
...
$localizadomaiusc=strtoupper($sessao);
...
echo'<hr size="10%" width="80%" color="#A2A6A9">';


echo'<table width="70%" bgcolor="'.$cor.'"  style="margin-left:5%;"><tr><td align="left"><h1>ENVIO DE MENSAGENS</h1>
Deixe sua Mensagem aqui !
<form name="coment" action="insercao.php" method="post"><br>

<textarea name="comentarios" cols="30" rows="6" scrolling="no" wrap="on"></textarea><br><br>


De <input type="hidden" name="sessao" size="20" value="';
?>
<?php
echo $login;

echo'" readonly="true"/>
Para <input type="hidden" name="postar" size="20" value="';
...
//$mensagens=@mysql_fetch_array($listar9);
//$mensagens1=$mensagens["comentario"]
...
echo'<br/><br/><table><td width="80%" colspan="2"  valign="top" align="left" cellspacing="1" >';
...
<a name="mensagens" style="text-decoration:none; font-size:15px;" ><b>MENSAGENS  DE '.strtoupper(str_replace("_"," ",$sessao)).'</b></a>&nbsp;&nbsp;
</div>';
...
$i=0;
for($i==0;$i<$linhas;$i++){
echo '<tr><id ><td  id="celula" align="left" cellspacing="2" valign="top"
style=" background:'.$cor.';" id="celula" height="50" align="left" >
<form method="post" action="excluir.php" id="coluna">
<font size="3" color="#0e3aa8" face="impact" style="text-transform:capitalize">
<input type="text" name="sessao" value="'.str_replace("_"," ",$nome=@mysql_result($listar9,$i)).'" readonly="true"
style="background:'.$cor.'; border:none; width:80%; height:40px; font-family:impact; color:#0e3aa8;
font-size:20px;"/>
...
style="background:'.$cor.'; border:none; width:80%; height:40px; font-family:calibri,arial; color:#0e3aa8;
...
<input type="submit" name="apagar" value="Apagar" style="float:right; margin-right:15px; border-radius:80px; background-color:white;"/>
</form>';
echo'</font><br/><br/>';
}
}
...
if ($tipo=="fornecedor"){
  ...
  $listar5=@mysql_query("SELECT telefone FROM  _fornecedores WHERE tituloPerfil LIKE '%$sessao%';",$conexao);
  ...
  $listar8=@mysql_query("SELECT imagem FROM _produtos WHERE imagem LIKE '%$sessao%';",$conexao);
  $listar9=@mysql_query("SELECT sessao FROM _mensagens WHERE postar = '$sessao' ORDER BY id DESC LIMIT 50;",$conexao);
  $listar10=@mysql_query("SELECT comentario FROM _mensagens WHERE postar = '$sessao' ORDER BY id DESC LIMIT 50;",$conexao);
...
$sql =  @mysql_query("SELECT * FROM _produtos WHERE fornecedor ='$localizadomaiusc' ORDER BY codigo DESC LIMIT 8 ; ",$conexao);
//echo $localizadomaiusc;
//$sql = @mysql_query('$sql');
//$limiteNum=@mysql_num_rows($limite);
//$i=0;

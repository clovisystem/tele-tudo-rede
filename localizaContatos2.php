...
$tipoPerfil=@mysql_query("SELECT * FROM ".$localizado.";",$conexao);
...
$listar8=@mysql_query("SELECT imagem FROM _image WHERE foto LIKE '%$localizado%';",$conexao);
$listar11=@mysql_query("SELECT dia FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar12=@mysql_query("SELECT mes FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar13=@mysql_query("SELECT telefone FROM _users WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
//echo $tipo;
//$listar21="Clovis_Jr";
...
<input type="text" readonly="true"  style="background:transparent; border:none; width:800px; font-size:180%;" name="Titulo"
...
//echo '<form>';
echo'<tr>
<td width="50%" colspan="1" align="left" valign="bottom" ></td><td>
<font color="#06438E" size="3" face="verdana"><b>Quem sou eu...</b></td></tr>';
...
echo'<td width="40%" rowspan="2" valign="top" style="border-radius:10px; margin-left:0px; width:20%;"
 id="celula1"><div style="border-radius:12px; background:#B0C6D2; margin-left:50px; width:20%;">
<textarea name="texto" readonly="true"  valign="bottom"  id="Texto" wrap="on" cols="35" rows="10" style="border-radius:10px; "
...
?>
<tr><td width="100%" colspan="3" height="100" valign="top" align="left" style="padding-left:20px; padding-bottom:20px;" >
<?php
...
if ($tipoPerfil2=="fornecedor"){


$listar0=@mysql_query("SELECT tituloPerfil FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%' ;",$conexao);
$listar1=@mysql_query("SELECT email FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar2=@mysql_query("SELECT textoPerfil FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar3=@mysql_query("SELECT atividades FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar4=@mysql_query("SELECT hobby FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar5=@mysql_query("SELECT status FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar6=@mysql_query("SELECT estado FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar7=@mysql_query("SELECT cidade FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar8=@mysql_query("SELECT imagem FROM _produtos WHERE foto LIKE '%$localizado%';",$conexao);
$listar11=@mysql_query("SELECT dia FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar12=@mysql_query("SELECT mes FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
$listar13=@mysql_query("SELECT telefone FROM _fornecedores WHERE tituloPerfil LIKE '%$_GET[nome]%';",$conexao);
//echo $tipo;
//$listar21="Clovis_Jr";
//$listar20=str_replace("_"," ",$listar0);
...
echo'<div style="width:98%; height:60px; background:black; color:#ad0a0a; border-radius:12px; padding-top:0px; padding-left:10px;">
<input type="text" readonly="true"  style="background:transparent; border:none; width:800px; font-size:180%; " name="Titulo"
id="Titulo" value="'.$perfil1=str_replace("_"," ",$perfil1);
echo'" />';
echo'<br/><br/>';
...
echo'<table  id="body" style="margin-left:0px; background:'.$cor.';border-radius:12px; width:98%;  ">
<tr><td colspan="3" style="padding-left:10px;">';
echo '<b>'.$voce.'</b><span style="font-size:18px; font-family:calibri,arial; color:green; font-weight:bold; ">, esta &eacute; a P&aacute;gina do perfil de <input type="text" name="logon" value="'.$localizado1=str_replace("_"," ",$localizado1).'" readonly="true" size="16" STYLE="background-color:transparent; border:none; font-size:18px; font-family:calibri,arial; color:green; font-weight:bold; margin-top:4px; margin-left:8px;"/></span>';
echo'</input>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="Voltar" onclick="history.go(-2);" />';
echo'</td></tr>';
...
<font color="#06438E" size="3" face="verdana" style="margin-left:170%;"><b>Quem sou eu...</b></td></tr>';
/*echo'<td width="15%" height="50" colspan="1" valign="bottom"><div id="campo" style="background-image:url(BACKGROUNDS/CampoMyWeb.png);
background-repeat:no-repeat;height:80px;padding-top:20px;padding-left:10px; ">E-mail:&nbsp;&nbsp;<font size="2" color="green" face="arial" style="font-weight:600;"> ' .@mysql_result($listar1,0,email);
echo'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font></div></td>';*/
...
$sql0 = "SELECT imagem FROM ".strtolower($localizado)." WHERE usuarios='".strtolower($localizado)."'";
$limite0 = @mysql_query("$sql0");
$localizado1=explode("_",$localizado);
//echo $localizado1[0];
while($sql0 = @mysql_fetch_array ($limite0)){
...
echo'<span style="float:left; position:relative; padding:23px 4px 4px 14px; font-weight:600;" ><a href='.$arquivo.'>
<img src="'.$arquivo.'" width="120"  height="150" border="2" bordercolor="#FF6600" style="border-radius:10px;" alt="Sua foto n&atilde;o p&ocirc;de ser carregada, tente outra."/></a>
<a href="'.$url.'"><button type="button" name="comprar" style="border-radius:80px; background-color:white;" />Comprar</button>
</a>
...
echo'<table width="98%" bgcolor="'.$cor.'" id="body" style="margin-left:8px;"><tr>
<td colspan="3"> <br/>';

$adicionar=@mysql_query("SELECT contatos FROM ".strtolower(str_replace(' ','_',$voce))." WHERE contatos LIKE'%".$localizado."%';",$conexao);
$adicionar1=@mysql_num_rows($adicionar);
/*echo $adicionar1;
echo strtolower(str_replace(" ","_",$voce));
echo $localizado;*/

if($adicionar1 > 0){ 
 echo'<div style="background:'.$cor.'; color:gold; width:100%; height:50px; border-radius:12px; padding-left:30px;">';
 echo"<b>Voces sao amigos.</b>";
 echo"</div>";
}
else{ 
	echo'<div id="formulario1">';
	echo'<form name="adiciona" method="post" action="adiciona.php?login='.$_GET["login"].'&tipo='.$tipo.'">';

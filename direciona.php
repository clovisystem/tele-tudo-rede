...
<BODY   style="background-image:url('BACKGROUNDS/Compras.jpg'); background-size:cover;background-repeat:no-repeat; 
background-attachment:fixed; height:100vh;" onload='submetendo()' >
...
<main  class="container container-fluid  col-xs-12 " > 
<BR/><BR/>
<div style="background-color:BLACK; border-radius:12px; width:100%; height:60px; "><p STYLE="width:100%; font-size:20px; color:#ad0a0a; padding-top:16px; padding-left:10px;">ENTRANDO...</p></div>
...
if($_POST['tipo']=="usuario"){ //ENTRADA DE USUARIO
$tipo=$_POST['tipo'];
...
$login=$_POST['c_email'];
$login=str_replace(" ","7",$login);
$login=str_replace("=","6",$login);
$login=str_replace("'","8",$login);
...
$perfil=@mysql_query("SELECT * FROM _users  WHERE _users.email ='$email' ;",$conexao);
//$empresario=@mysql_query("SELECT * FROM _fornecedor  WHERE email ='$email';",$conexao);
//$perfil=preg_replace('/[^[:alpha:]_]/', '',$perfil);   //evita SQL INJECTION   com letras
$perfil1=@mysql_num_rows($perfil);
//$empresario1=@mysql_num_rows($empresario);
...
$redireciona=@mysql_query("SELECT * FROM _users WHERE _users.email ='$email' ;",$conexao);
$redireciona=@mysql_fetch_array($redireciona);  
$redireciona=$redireciona["tipo"];

/*$redireciona1=@mysql_query("SELECT * FROM _fornecedores WHERE _users.email ='$email' ;",$conexao);
$redireciona1=@mysql_fetch_array($redireciona);  
$redireciona1=$redireciona1["tipo"];*/

 
//echo $redireciona; //echo $perfil;

/*if($redireciona1){
  echo"<form  name='form0' id='form0' method='post'  action='empresas/".$perfil."/index.php'>";
  echo"Aguarde...";
...
if($redireciona){
  echo"<form  name='form0' id='form0' method='post'  action='usuarios/".$perfil."/index.php?tipo=".$tipo."'>";
  echo"Aguarde...";
  echo'<input type="hidden" name="c_senha"  value="'.$password.'"/>';
  echo'<input type="hidden" name="c_email"  value="'.$login.'"/>';
  echo'<input type="hidden" name="dominio"  value="'.$dominio.'"/>';
...
if($_POST["tipo"]=="fornecedor"){ //ENTRADA DE FORNECEDOR
  $tipo=$_POST["tipo"];
  $password=$_POST["c_senha1"];
  $password=str_replace(" ","7",$password);
  $password=str_replace("=","6",$password);
  $password=str_replace("'","8",$password);
  //$password=preg_replace('/[^[:alnum:]_]/', '',$password);  //evita SQL INJECTION  com numeros
  ...
  $perfil=@mysql_query("SELECT * FROM _fornecedores  WHERE _fornecedores.email ='$email' ;",$conexao);
  //$empresario=@mysql_query("SELECT * FROM _fornecedor  WHERE email ='$email';",$conexao);
  //$perfil=preg_replace('/[^[:alpha:]_]/', '',$perfil);   //evita SQL INJECTION   com letras
  $perfil1=@mysql_num_rows($perfil);
  //$empresario1=@mysql_num_rows($empresario);
  ...
  $redireciona=@mysql_query("SELECT * FROM _fornecedores WHERE _fornecedores.email ='$email' ;",$conexao);
  $redireciona=@mysql_fetch_array($redireciona);  
  $redireciona=$redireciona["tipo"];
  ...
  if($redireciona){
    echo"<form  name='form0' id='form0' method='post'  action='empresas/".$perfil."/index.php?tipo=".$tipo."'>";
    echo"Aguarde...";
    echo'<input type="hidden" name="c_senha"  value="'.$password.'"/>';
    echo'<input type="hidden" name="c_email"  value="'.$login.'"/>';
    echo'<input type="hidden" name="dominio"  value="'.$dominio.'"/>';
